...
const FileUploader: React.FC = () => {
    ...
    const handleUpload = async (file: File | null, endpoint: string, keyName: string , setLoadingState: React.Dispatch<React.SetStateAction<boolean>> ) => { 
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert(`Upload de ${endpoint} bem-sucedido: ${result.details || result.message}`);
    ...
    return (
        <div className="flex gap-6 justify-center  w-full h-1 mx-auto max-w-4xl p-4  flex-shrink-0"> 
            {/* --------------------------- */}
            {/* BOX 1: Arquivo de Usuários  */} 
            {/* --------------------------- */}
            <div className="flex-1 bg-white p-6 rounded-lg shadow-lg border border-gray-200 min-h-fit flex-shrink-0">
                <h2 className="text-xl font-semibold text-gray-800 mb-4 border-b pb-2">Subir Arquivo de Usuários</h2>
                
                <label
                    htmlFor="dropzone-users" 
                    className="flex flex-col justify-center items-center w-full h-48 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 cursor-pointer hover:bg-gray-100 transition duration-150 flex-shrink-0 "
                >
                    {/* ... Ícone e Texto ... */}
                    <div className="fill">
                      <svg
                ...
                {/* 3. CHAMADA DO BOTÃO DE USUÁRIOS */}
                <button
